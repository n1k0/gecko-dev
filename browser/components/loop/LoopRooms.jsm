...
// are retrieved from the server, this is list may become out of date. The Push
// server may notify us of this event, which will set the global 'dirty' flag to
// TRUE.
let gDirty = true;
let gRooms = new Map();
...
let LoopRoomsInternal = {
  ...
  getAll: function(version = null, callback) {
    ...
    Task.spawn(function* () {
      ...
      for (let room of roomsList) {
        let eventName = gRooms.has(room.roomToken) ? "update" : "add";
        gRooms.set(room.roomToken, room);
        try {
        ...
        eventEmitter.emit(eventName, gRooms.get(room.roomToken));
      }
  ...
  get: function(roomToken, callback) {
    let room = gRooms.has(roomToken) ? gRooms.get(roomToken) : {};
